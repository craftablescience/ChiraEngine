...
EditorCamera3d::EditorCamera3d(Entity* parent_, CameraProjectionMode mode, float fov_, bool controls) : Camera3d(parent_, mode, fov_) {
    if (controls)
        EditorCamera3d::setupKeybinds();
}

EditorCamera3d::EditorCamera3d(Entity* parent_, const std::string& name_, CameraProjectionMode mode, float fov_, bool controls) : Camera3d(parent_, mode, fov_) {
    if (controls)
        EditorCamera3d::setupKeybinds();
}

EditorCamera3d::EditorCamera3d(CameraProjectionMode mode, float fov_, bool controls) : Camera3d(mode, fov_) {
    if (controls)
        EditorCamera3d::setupKeybinds();
}

EditorCamera3d::EditorCamera3d(const std::string& name_, CameraProjectionMode mode, float fov_, bool controls) : Camera3d(name_, mode, fov_) {
    if (controls)
        EditorCamera3d::setupKeybinds();
}
...
void EditorCamera3d::setupKeybinds() {
    Engine::addKeybind(Keybind(GLFW_KEY_W, GLFW_REPEAT, []{
        if (!Engine::isMouseCaptured())
            return;
        if (auto cam = assert_cast<EditorCamera3d*>(Engine::getRoot()->getMainCamera()))
