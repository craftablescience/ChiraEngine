...
image::image(const unsigned char* buffer, unsigned int bufferLen, int* width, int* height, int* fileChannels, int desiredChannels, bool vflip) : abstractImage() {
...
image::image(const unsigned char* buffer, unsigned int bufferLen, int desiredChannels, bool vflip) {
    stbi_set_flip_vertically_on_load(vflip);
